...
function SVGSpriteBase(spriter, config, data, key) {
    ...
    var spritePath = ('spritePath' in this.config) ? this.config.spritePath + '/' + spriteName : path.relative(this._cssDest, this.config.sprite).split(path.sep).join('/');
...
SVGSpriteBase.prototype._addCacheBusting = function(svg) {
	...
	if (this.config.bust) {
        ...
        if (this.config.spritePath != undefined) {
            this.data.spritePath    = this.config.spritePath + '/' + path.basename(sprite, extension) + extension;
        } else {
            this.data.spritePath    = path.relative(this._cssDest, sprite).split(path.sep).join('/');
        }
